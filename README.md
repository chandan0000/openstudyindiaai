...
Each generated project includes a CLI tool named `openstudyindiaai`. Run commands from the `backend/` directory:

```bash
cd backend

# Server commands
uv run openstudyindiaai server run --reload     # Start dev server
uv run openstudyindiaai server routes           # Show all routes
# Database commands
uv run openstudyindiaai db migrate -m "message" # Create migration
uv run openstudyindiaai db upgrade              # Apply migrations
uv run openstudyindiaai db downgrade            # Rollback migration
# User commands
uv run openstudyindiaai user create-admin       # Create admin user
uv run openstudyindiaai user create             # Create regular user
uv run openstudyindiaai user list               # List all users
```
...
# Server
uv run openstudyindiaai server run --reload
uv run openstudyindiaai server routes

# Database (Alembic wrapper)
uv run openstudyindiaai db init
uv run openstudyindiaai db migrate -m "Add users"
uv run openstudyindiaai db upgrade

# Users
uv run openstudyindiaai user create-admin       # Create admin (interactive)
uv run openstudyindiaai user create             # Create user (interactive)
uv run openstudyindiaai user list               # List all users
```
...
```bash
uv run openstudyindiaai cmd seed --count 100
uv run openstudyindiaai cmd seed --dry-run
```
...
| Start dev server | `uv run --directory backend openstudyindiaai server run --reload` |
| Start prod server | `uv run --directory backend openstudyindiaai server run --host 0.0.0.0 --port 8000` |
| Show routes | `uv run --directory backend openstudyindiaai server routes` |
...
| Task | Command |
|------|---------|
| Create migration | `uv run --directory backend openstudyindiaai db migrate -m "message"` |
| Apply migrations | `uv run --directory backend openstudyindiaai db upgrade` |
| Rollback migration | `uv run --directory backend openstudyindiaai db downgrade` |
| Show current | `uv run --directory backend openstudyindiaai db current` |
| Show history | `uv run --directory backend openstudyindiaai db history` |

### Users

| Task | Command |
|------|---------|
| Create admin | `uv run --directory backend openstudyindiaai user create-admin` |
| Create user | `uv run --directory backend openstudyindiaai user create` |
| List users | `uv run --directory backend openstudyindiaai user list` |

### Celery

| Task | Command |
|------|---------|
| Start worker | `uv run --directory backend openstudyindiaai celery worker` |
| Start beat | `uv run --directory backend openstudyindiaai celery beat` |
| Start flower | `uv run --directory backend openstudyindiaai celery flower` |
