...
db-init: docker-db
	...
	@sleep 3
	uv run --directory backend openstudyindiaai db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend openstudyindiaai db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend openstudyindiaai db upgrade

db-downgrade:
	uv run --directory backend openstudyindiaai db downgrade

db-current:
	uv run --directory backend openstudyindiaai db current

db-history:
	uv run --directory backend openstudyindiaai db history

# === Server ===
run:
	uv run --directory backend openstudyindiaai server run --reload

run-prod:
	uv run --directory backend openstudyindiaai server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend openstudyindiaai server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend openstudyindiaai user create-admin

user-create:
	uv run --directory backend openstudyindiaai user create

user-list:
	uv run --directory backend openstudyindiaai user list

# === Celery ===
celery-worker:
	uv run --directory backend openstudyindiaai celery worker

celery-beat:
	uv run --directory backend openstudyindiaai celery beat

celery-flower:
	uv run --directory backend openstudyindiaai celery flower
	@echo ""
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/openstudyindiaai"
...
help:
	@echo ""
	@echo "openstudyindiaai - Available Commands"
	@echo "======================================"
	@echo ""
	@echo "Setup:"
